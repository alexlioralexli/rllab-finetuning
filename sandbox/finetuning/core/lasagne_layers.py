...
# this class enables having many latents, and makes the optimization easier by not having one set
class BilinearIntegrationLayerAllLatents(L.MergeLayer):
    def __init__(self, incomings, name=None):
        super(BilinearIntegrationLayerAllLatents, self).__init__(incomings, name)
        self.num_latents = len(incomings) - 1  # elements of incomings, from index 1 on, should be latents
    ...
    def get_output_shape_for(self, input_shapes):
        ...
        if n_batch is None:
            ...
            return n_batch * self.num_latents, robot_dim + selection_dim + robot_dim * selection_dim

    def get_output_for(self, inputs, **kwargs):
        obs_robot_var = inputs[0]
        selection_vars = inputs[1:]

        bilinears = [TT.concatenate([obs_robot_var, selection_var, TT.flatten(obs_robot_var[:, :, np.newaxis] *
                            selection_var[:, np.newaxis, :], outdim=2)], axis=1) for selection_var in selection_vars]
        return TT.concatenate(bilinears, axis=0)
