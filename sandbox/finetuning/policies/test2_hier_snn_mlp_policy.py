...
class GaussianMLPPolicy_snn_hier(StochasticPolicy, LasagnePowered, Serializable):  # also inherits from Parametrized
    ...
    def __init__(
        ...
        assert self.external_latent
        l_all_obs_var_with_lat = L.InputLayer(shape=(None,) + (self.obs_robot_dim + self.obs_maze_dim + self.latent_dim,))
        all_obs_var = l_all_obs_var_with_lat.input_var
        ...
        # l_selection = l_obs_maze = CropLayer(l_all_obs_var_with_lat, start_index=self.obs_robot_dim + self.obs_maze_dim, end_index=None)
        l_selection = CropLayer(l_all_obs_var_with_lat, start_index=self.obs_robot_dim + self.obs_maze_dim, end_index=None)
        # split all_obs into the robot and the maze obs --> ROBOT goes first!!
        l_obs_robot = CropLayer(l_all_obs_var_with_lat, start_index=None, end_index=self.obs_robot_dim)
        l_obs_maze = CropLayer(l_all_obs_var_with_lat, start_index=self.obs_robot_dim, end_index=self.obs_robot_dim + self.obs_maze_dim)
    ...
    def set_params_snn(self, snn_params):
        ...
        for param in local_params:
            # if param.name != "output_log_std.param":  #todo: remove this later in case
            #     param.set_value(params_value_by_name[param.name])
            param.set_value(params_value_by_name[param.name])
    def dist_info_sym(self, obs_var, state_info_var=None):  # put latent into state_info_var
        mean_var, log_std_var = L.get_output([self._l_mean, self._l_log_std], TT.concatenate((obs_var, state_info_var), axis=1))
        if self.min_std is not None:
            log_std_var = TT.maximum(log_std_var, np.log(self.min_std))
    ...
    @overrides
    def get_action(self, observation):  #note: observation should be a tuple of (observation, latents)
        assert len(observation) == 2
        actions, outputs = self.get_actions([observation[0]], [observation[1]])
    ...
    def get_actions(self, observations, latents):
        assert len(observations) == len(latents)
        mean, log_std = self._f_dist(np.concatenate((observations, latents), axis=1))
