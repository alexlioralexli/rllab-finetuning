...
from sandbox.finetuning.core.lasagne_layers import BilinearIntegrationLayer, BilinearIntegrationLayerAllLatents, CropLayer
...
class GaussianMLPPolicy_snn_hier(StochasticPolicy, LasagnePowered, Serializable):  # also inherits from Parametrized
    ...
    def __init__(
            ...
            npz_path=None,
            trainable_snn=True,
            trainable_latents=False,  # whether the latent vectors themselves can be trained
        ...
        # for doing all the calculations concurrently:
        if latents is None:
            mat = np.eye(latent_dim)
            latents = [mat[i, :] for i in range(latent_dim)]
        self.shared_latents = [theano.shared(latent) for latent in latents]
        ...
        self.trainable_latents = trainable_latents
        ...
        if isinstance(env, MazeEnv) or isinstance(env, GatherEnv):
            ...
            if isinstance(env.wrapped_env, MazeEnv) or isinstance(env.wrapped_env, GatherEnv):
                ...
                self.obs_maze_dim = 0
        elif isinstance(env, PeriodVaryingEnv):  #todo: fix this
            if isinstance(env.wrapped_env, MazeEnv) or isinstance(env.wrapped_env, GatherEnv):
                self.obs_robot_dim = env.wrapped_env.robot_observation_space.flat_dim + 1
                self.obs_maze_dim = env.wrapped_env.maze_observation_space.flat_dim
            elif isinstance(env.wrapped_env, NormalizedEnv):
                if isinstance(env.wrapped_env.wrapped_env, MazeEnv) or isinstance(env.wrapped_env.wrapped_env, GatherEnv):
                    self.obs_robot_dim = env.wrapped_env.wrapped_env.robot_observation_space.flat_dim + 1
                    self.obs_maze_dim = env.wrapped_env.wrapped_env.maze_observation_space.flat_dim
                else:
                    self.obs_robot_dim = env.wrapped_env.observation_space.flat_dim + 1
                    self.obs_maze_dim = 0
        else:
        ...
        if self.external_latent:  # in case we want to fix the latent externally
            ...
            l_selection = ParamLayer(incoming=l_all_obs_var, num_units=self.latent_dim, param=self.shared_latent_var,
                                     trainable=False)
            # alex
            l_selections = [ParamLayer(incoming=l_all_obs_var, num_units=self.latent_dim, param=latent_var,
                                     trainable=self.trainable_latents) for latent_var in self.shared_latents]
            ...
            # alex
            l_selections = [ParamLayer(incoming=l_all_obs_var, num_units=self.latent_dim, param=latent_var,
                                       trainable=False) for latent_var in self.shared_latents]
        ...
        if self.bilinear_integration:
            ...
            l_obs_snn = BilinearIntegrationLayerAllLatents([l_obs_robot] + l_selections) #todo
        else:
        ...
        if self.json_path and self.npz_path:
            ...
            data = joblib.load(os.path.join(config.PROJECT_PATH, self.pkl_path))
            warm_params = data['policy'].get_params_internal() #todo: verify that this works correctly
        ...
        self.first_latent = ext.compile_function(
            inputs=[],
            outputs=self.shared_latents[0]
        )
    ...
    def dist_info_sym(self, obs_var, state_info_var=None):
        # first case added by alex to interface with vpg directly
        if self.latent_dim == 1:
            return self.dist_info_sym_all_latents(obs_var, state_info_var=state_info_var)[0]
        else: # dont use
    ...
    def dist_info_sym_all_latents(self, obs_var, state_info_var=None):
        mean_vars, log_std_vars = L.get_output([self._l_mean, self._l_log_std], obs_var)
        if self.min_std is not None:
            log_std_vars = TT.maximum(log_std_vars, np.log(self.min_std))

        length = mean_vars.shape[0] // len(self.shared_latents)
        dist_infos = [dict(mean=mean_vars[i*length:(i+1)*length], log_std=log_std_vars[i*length:(i+1)*length])
                      for i in range(len(self.shared_latents))]
        # return dict(mean=mean_var, log_std=log_std_var)
        return dist_infos

    # for use in concurrent_train.py
    # unnecessary now
    def set_latent_train(self, latent):
    ...
    # todo: check if i select the actions correctly here
    def get_actions(self, observations):

        selector_output = self._f_select(observations)
        means, log_stds = self._f_dist(observations)

        length = means.shape[0]//len(self.shared_latents)
        mean = means[length*self.current_latent:length*(self.current_latent + 1), :]
        log_std = log_stds[length*self.current_latent:length*(self.current_latent + 1), :]
    ...
    def set_pre_fix_latent(self, latent):
        self.pre_fix_latent = np.array(latent)
        self.current_latent = np.argmax(self.pre_fix_latent)
    ...
    #todo: for the npz saving, highly hacky
    def get_params(self, **tags):  # adds the list to the _cached_params dict under the tuple key (one)
        ...
        if len(tags) == 1 and 'hacky_npz' in tags:
            ...
            return self._cached_params[tag_tuple][self.latent_dim:]
        tag_tuple = tuple(sorted(list(tags.items()), key=lambda x: x[0]))
        if tag_tuple not in self._cached_params:
            self._cached_params[tag_tuple] = self.get_params_internal(**tags)
