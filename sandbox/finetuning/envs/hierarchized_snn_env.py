...
from sandbox.finetuning.policies.hier_snn_mlp_policy import GaussianMLPPolicy_snn_hier
...
class HierarchizedSnnEnv(ProxyEnv, Serializable):
    ...
    def step(self, action):
        ...
        self.r += reward
        if done:
            if 'episode' in last_agent_info:
                import ipdb; ipdb.set_trace()
                last_agent_info['episode']['r'] = self.r
                last_agent_info['episode']['l'] = self.t  # length of rollout
            else:
                last_agent_info['episode'] = dict(r=self.r, l=self.t)
