...
def make_plot(plot_list, use_median=False, plot_width=None, plot_height=None, title=None):
    ...
    for idx, plt in enumerate(plot_list):
        ...
        data.append(go.Scatter(
            ...
            line=go.scatter.Line(color=core.hex_to_rgb(color, 0)),  # line=go.Line(color='transparent'),
            showlegend=False,
...
def plot_div():
    ...
    plot_div = get_plot_instruction(plot_key=plot_key, split_key=split_key, filter_nan=filter_nan,
                                    ...
                                    custom_series_splitter=lambda exp: exp['flat_params']['exp_name'].rsplit('_', 1)[0]) # custom series splitter
    # print plot_div
    return plot_div
...
def index():
    ...
    plot_div = get_plot_instruction(
        plot_key=plot_key, split_key=None, group_key=group_key, custom_series_splitter=lambda exp: exp['flat_params']['exp_name'].rsplit('_', 1)[0]) #added the custom series splitter
